...
//give div for bubble chart from html
...
function populatePieChart(sample_otu_distribution){
    ...
    //select class div from html to put data 
...
function populateBackground(personal_data){
    ...
    .append('tr')
    .html(function(d){
        return `<td>${d[0]}</td><td>${d[1]}</td>` //putting in every td data key and value from personlinfo dict
    })
}
...
Plotly.d3.json("/names",function(error,response){
    ...
    console.log(response);
    Plotly.d3.select("#sample-select")
    .append("option")
    .attr("selected","true")
    .attr("disabled","true")
